  ...
  return (
    <div className='p-6 max-w-7xl mx-auto space-y-6'>
      <div className='flex flex-col md:flex-row justify-between items-start md:items-center gap-4'>
        <div>
          <h1 className='text-2xl font-bold tracking-tight'>Thư viện Quiz</h1>
          <p className='text-muted-foreground'>
            Quản lý và tham gia các bài kiểm tra của bạn.
          </p>
        </div>
        <Button onClick={() => router.push('/quizzes/new')}>
          <Plus className='mr-2 size-4' /> Tạo Quiz mới
        </Button>
      </div>
              ...
              <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4'>
                ...
                {filteredQuizzes?.length === 0 && (
                  <div className='col-span-full py-12 text-center border-2 border-dashed rounded-lg bg-slate-50/50'>
                    <p className='text-muted-foreground'>
                      {tab === 'my-quizzes'
                        ? 'Bạn chưa tạo bài quiz nào.'
                        : tab === 'joined'
                          ? 'Bạn chưa tham gia bài quiz nào.'
                          : 'Hiện không có bài quiz công khai nào.'}
                    </p>
                  </div>
                )}
              </div>
