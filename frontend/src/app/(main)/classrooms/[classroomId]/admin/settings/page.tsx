...
export default function AdminSettingsPage() {
  ...
  const handleDelete = async () => {
    if (
      ...
      return;
    try {
      toast.info('Đang thực hiện xóa lớp học...');
      await deleteClassroom(classroomId);
      router.push('/classrooms');
      toast.success('Đã xóa lớp học vĩnh viễn!');
    } catch (e: any) {
    ...
    <div className='bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden'>
      <div className='p-6 border-b border-slate-100 bg-slate-50'>
        ...
        <p className='text-slate-500 text-sm mt-1'>
          Thay đổi thông tin cơ bản hoặc xóa lớp học
        </p>
      </div>
      ...
      <div className='p-6 space-y-6 max-w-2xl'>
        ...
        <div className='pt-8 border-t border-red-100 mt-8'>
          ...
          <p className='text-sm text-slate-500 mb-4'>
            Hành động xóa lớp học sẽ xóa vĩnh viễn toàn bộ dữ liệu, thành viên,
            và tài nguyên của lớp. Không thể khôi phục.
          </p>
          <button
