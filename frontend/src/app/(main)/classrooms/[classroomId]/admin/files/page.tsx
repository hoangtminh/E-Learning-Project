...
export default function AdminFilesPage() {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // 4. Create systemic notification post in group feed
      if (confirmRes.success && confirmRes.data?.id) {
        try {
          await createPost(
            classroomId,
            `[SYSTEM_FILE]:${confirmRes.data.id}:${file.name}`,
          );
        } catch (postErr) {
          console.error(
            'Failed to create systemic announcement post for file:',
            postErr,
          );
        }
      }
      ...
      <div className='p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50'>
        <div>
          ...
          <p className='text-slate-500 text-sm mt-1'>
            Thêm, sửa, xóa tài liệu của lớp học
          </p>
        </div>
        <div>
          <input
            ...
            <table className='w-full text-left text-sm text-slate-600'>
              <thead className='bg-slate-50 text-slate-500 border-b border-slate-200'>
                <tr>
                  ...
                  <th className='px-4 py-3 font-semibold text-right rounded-tr-xl'>
                    Hành động
                  </th>
                </tr>
              </thead>
