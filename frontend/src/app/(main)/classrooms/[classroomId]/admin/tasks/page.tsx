...
function TaskFormModal({
  ...
  const { createTask, updateTask, getTaskAttachmentPresignedUpload } = useTasks();
    ...
    try {
      ...
      if (attachFile) {
        ...
        if (!isEdit) {
          ...
          const tempTask = await new Promise<ClassroomTask>((resolve, reject) => {
            import('@/api/classroom').then(({ createTask: apiCreate }) => {
              apiCreate(classroomId, { title, description: description || undefined, deadline: deadline || undefined })
                .then((r) => r.success && r.data ? resolve(r.data) : reject(new Error(r.error || 'Tạo bài tập thất bại')))
                .catch(reject);
            });
          });
          const { url, s3Key } = await getTaskAttachmentPresignedUpload(
            classroomId, tempTask.id, attachFile.name, attachFile.type || 'application/octet-stream'
          );
          await fetch(url, { method: 'PUT', body: attachFile, headers: { 'Content-Type': attachFile.type || 'application/octet-stream' } });
          await updateTask(classroomId, tempTask.id, { attachmentKey: s3Key, attachmentName: attachFile.name });
          setUploadingAttach(false);
          onClose();
          return;
        }
        const { url, s3Key } = await getTaskAttachmentPresignedUpload(
          classroomId, editingTask!.id, attachFile.name, attachFile.type || 'application/octet-stream'
        );
        await fetch(url, { method: 'PUT', body: attachFile, headers: { 'Content-Type': attachFile.type || 'application/octet-stream' } });
        attachmentKey = s3Key;
      ...
      if (isEdit) {
        await updateTask(classroomId, editingTask!.id, {
          title, description: description || undefined, deadline: deadline || null,
          ...(attachmentKey && { attachmentKey, attachmentName }),
        });
      } else {
        await createTask(classroomId, { title, description: description || undefined, deadline: deadline || undefined });
      }
      onClose();
    } catch (e: any) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl p-6 w-full max-w-lg shadow-2xl">
        ...
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">Mô tả / Đề bài</label>
            <textarea
              value={description}
          ...
          {/* Attachment file */}
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">File đính kèm đề bài</label>
            <div
            ...
            >
              <span className="material-symbols-outlined text-2xl text-slate-400 block mb-1">attach_file</span>
              {attachFile ? (
                <p className="text-sm font-semibold text-indigo-600">{attachFile.name}</p>
              ) : hasExistingAttachment ? (
                <p className="text-sm text-slate-500">
                  Hiện tại: <span className="font-semibold text-slate-700">{editingTask?.attachmentName}</span><br />
                  <span className="text-xs text-slate-400">Chọn file mới để thay thế</span>
                </p>
              ) : (
                <p className="text-sm text-slate-500">Nhấn để đính kèm file đề bài (PDF, DOCX...)</p>
              )}
            </div>
            <input ref={attachInputRef} type="file" className="hidden" onChange={(e) => setAttachFile(e.target.files?.[0] ?? null)} />
            {uploadingAttach && <p className="text-xs text-indigo-500 mt-1 animate-pulse">Đang upload file...</p>}
          </div>
...
function SubmissionsPanel({
  ...
  const handleGrade = async (submissionId: string) => {
    ...
    try {
      ...
      alert(e.message || 'Chấm điểm thất bại');
    } finally {
  ...
  const handleDownload = async (submissionId: string) => {
    ...
    try {
      const url = await getSubmissionDownloadUrl(classroomId, task.id, submissionId);
      window.open(url, '_blank');
    } catch (e: any) {
      alert(e.message || 'Không thể tải file');
    } finally {
    ...
    <div className="flex-1 min-h-0 overflow-y-auto">
      ...
      {loading ? (
        ...
        <div className="text-center py-12 bg-slate-50 rounded-xl border border-dashed border-slate-200">
          ...
          <p className="text-slate-500 text-sm font-medium">Chưa có học sinh nào nộp bài</p>
        </div>
      ) : (
            ...
            return (
              ...
              >
                <div className="flex gap-3 items-start">
                  <Avatar name={sub.user.fullName} avatarUrl={sub.user.avatarUrl} />
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="mt-3 flex items-center gap-2">
                      <input
                        ...
                        onChange={(e) =>
                          setGrading((p) => ({ ...p, [sub.id]: e.target.value }))
                        }
...
export default function AdminTasksPage() {
  ...
  const handleDelete = async (task: ClassroomTask) => {
    if (!confirm(`Xóa bài tập "${task.title}"? Tất cả bài nộp sẽ bị xóa theo.`)) return;
    try {
  ...
  const totalSubmissions = tasks.reduce((acc, t) => acc + (t._count?.submissions || 0), 0);

  return (
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col" style={{ minHeight: 600 }}>
      {/* Header */}
        ...
        <div className="w-80 shrink-0 flex flex-col overflow-y-auto">
          {loadingTasks ? (
            ...
            <div className="text-center py-16 m-4 bg-slate-50 rounded-xl border border-dashed border-slate-200">
              ...
              <p className="text-slate-500 text-sm font-medium">Chưa có bài tập nào.</p>
            </div>
          ) : (
                          ...
                          {task.deadline && (
                            <span
                              className={`text-xs flex items-center gap-0.5 ${
                                isPast ? 'text-slate-400 line-through' : 'text-amber-600'
                              }`}
                            >
        ...
        <div className="flex-1 p-6 overflow-y-auto flex flex-col">
          {selectedTask ? (
            ...
            <div className="flex flex-col items-center justify-center h-full text-slate-400 select-none">
              ...
              <p className="font-medium text-sm">Chọn một bài tập để xem bài nộp</p>
              <p className="text-xs mt-1 opacity-70">Nhấn vào tên bài tập ở bên trái</p>
            </div>
          )}
        </div>
      </div>
      ...
      {/* Modals */}
      {showCreateModal && (
        <TaskFormModal classroomId={classroomId} onClose={() => setShowCreateModal(false)} />
      )}
      {editingTask && (
        <TaskFormModal
